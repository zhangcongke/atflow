...
```bash
curl -fsSL https://raw.githubusercontent.com/zhangcongke/atflow/main/scripts/install.sh | bash
```
...
`ATFLOW_INSTALL_DIR` should point to a `bin` directory. Cargo installs to `<root>/bin` when `--root <root>` is used, so the script derives the install root from that final bin path.
